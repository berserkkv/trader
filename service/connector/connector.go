...
func GetPrice(smb symbol.Symbol) float64 {
...
func GetKlines(smb symbol.Symbol, interval timeframe.Timeframe, limit int) []model.Candle {

	url := fmt.Sprintf("https://fapi.binance.com/fapi/v1/klines?symbol=%s&interval=%s&limit=%d", string(smb), interval, limit)
	slog.Debug("GetKlines", "url", url)
