...
func (s HAEMAStrategy) Run(candles []model.Candle) (order.Command, string) {
	...
	info := fmt.Sprintf("price=%.4f, ema=%.4f, HAColor=%s, HAChanged=%t",
		candles[len(candles)-1].Close, ema40, color, changed)

	slog.Debug(s.Name(), "changed", changed, "color", color, "ema", ema40, "price", candles[len(candles)-1].Close)
	if changed {
		if color == "green" && candles[len(candles)-1].Close > ema40 {
			return order.LONG, "LONG " + info
		} else if color == "red" && candles[len(candles)-1].Close < ema40 {
